/*
 ...
 By pressing button #2 it helps prepare milk for cheese
...
//signalling that cheese temp is reached
void pitch_cheese(){
  //print operation
  lcd.clear();
  lcd.home();
  lcd.print("Pitch Cheese");
  lcd.setCursor(0, 1);
  lcd.print("1 to continue");
  //flash the backlight to get attention
  int time_elapsed = millis();
...
void make_yoghurt(){
  //display message saying what you're doing
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print("Make Yoghurt!");
  delay(2000);  
...
void make_cheese(){
  ...
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print("Make Cheese!");
  delay(2000);  
  //heat milk to 185 degrees
  heat_up(185);
  //pastuerize milk for 30 minutes
  maintain_temp(185, 30);
  //cool the milk down to 85 degrees
  cool_down(85);
  //request that the user pitch the culture
  pitch_cheese();
  //hold the temp at 85 for 3 hours
  maintain_temp(85, 180);
}
